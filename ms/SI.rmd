...
d.f., degrees of freedom; MS, mean squares. Response variables include mean, minimum ('min'), maximum ('max'), and standard deviation ('sd') of temperature ('temp', °C) and vapor pressure deficit ('vpd', kPa). 'NA' indicates not applicable.
...
footer <- "d.f., degrees of freedom; MS, mean squares. Response variables include mean, minimum ('min'), maximum ('max'), and standard deviation ('sd') of temperature ('temp', °C) and vapor pressure deficit ('vpd', kPa). 'NA' indicates not applicable." %>% wrap_quotes

# Write out table with metadata
filename <- s_table("ancova") %>%
  str_replace_all(" ", "_") %>%
  paste0(".csv")
...
header <- c(
  si_table_header,
  s_table_nums("linear-fit") %>% wrap_quotes
)

footer <- "MPDphy, standard effect size of mean phylogenetic distance; MNTDphy, standard effect size of mean nearest taxon distance; MPDfunc, standard effect size of mean functional distance; MNTDfunc, standard effect size of mean nearest functional distance; VPD, vapor pressure deficit, SD, standard deviation; SLA, specific leaf area; AICc, corrected Akaike Information Criterion; d.f., degrees of freedom; P_moran, P value for the null hypothesis that there is spatial autocorrelation in the residuals as measured with Moran's I. Trait names refer to community-weighted mean values (see Methods). For details on calculations of MPD and MNTD, see Methods. 'NA' indicates not applicable." %>% wrap_quotes

filename <- s_table("linear-fit") %>%
  str_replace_all(" ", "_") %>%
  paste0(".csv")
...
# Write out table with metadata
filename <- s_table("phylosig-binary-strict") %>%
  str_replace_all(" ", "_") %>%
  paste0(".csv")
...
# Write out table with metadata
filename <- s_table("corr-evo-strict") %>%
  str_replace_all(" ", "_") %>%
  paste0(".csv")
...
header <- c(
  si_table_header,
  s_table_nums("gam-fit") %>% wrap_quotes
)
...
Spatial autocorrelation accounted for by including a Mátern spatial correlation structure in model (see Methods).

```{r spatial-sla}
# Format table
spatial_sla_formatted <-
div_el_model_parameter_summaries %>%
  filter(var == "sla") %>%
...
header <- c(
  si_table_header,
  s_table_nums("spatial-sla") %>% wrap_quotes
)

footer <- "Spatial autocorrelation accounted for by including a Mátern spatial correlation structure in model (see Methods)." %>%
  str_wrap(width = 100) %>%
  str_split("\\n") %>%
  unlist %>%
  purrr::map_chr(wrap_quotes)
