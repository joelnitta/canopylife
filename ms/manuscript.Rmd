...
loadd(correlated_evo_test)
loadd(diversity_data)
loadd(fern_traits)
loadd(model_summaries)
loadd(pca_plot)
loadd(pca_results)
...
We calculated mean daily maximum, mean, minimum, and standard variation of temperature, RH, and VPD for each site from the raw climate data, then grand means from the daily means. We analyzed correlation between the grand means with Pearson's correlation coefficient, and retained a set with correlation coefficients less than 0.9 (hereafter, "climatic variables"). We tested for differences between epiphytic and terrestrial dataloggers while accounting for elevation with ANCOVA, using growth habit as the categorical variable and elevation as the covariate for each climatic response variable. For visualizing the results, we selected the linear model with the lowest Akaike Information Criterion (AIC) out of the set of models including interaction effects and the effect of each independent variable alone. A single outlier site located on an exposed south-facing slope with unusually high temperature and low humidity (Mt. Rotui, 830 m) site was excluded from analysis.
...
Prior to testing for differences in diversity metrics, we first checked for correlation between metrics with Pearson's correlation coefficient, and retained a set of metrics with absolute correlation coefficients < 0.9. We then tested for differences between epiphytic and terrestrial communities with a paired t-test. We also conducted a more thorough analysis simultaneously testing the effects of environment and growth habit on diversity metrics using a full-subsets multiple regression approach with general additive models (GAMs) as implemented in the \package{FSSgamm} R package [@Fisher2018]. For each diversity metric, we built a set of models (Gaussian family, with site as a random effect) taking into account all pairwise interactions of explanatory variables, and ranked these by corrected AIC (AICc) to find the models that best explain the data. Our dataset included `r nrow(diversity_data) - nrow(grand_mean_climate_select)` communities that lacked climate data and could not be included in full-subsets multiple regression analysis. Since we found that all environmental variables are related to elevation (see Results), we also constructed linear models each diversity metric by growth habit with elevation as the covariate.
...
Mean and standard deviation of temperature and relative humidity were retained after excluding variables with correlation coefficients >0.9.
...
t_test_results_print <-
  t_test_results %>%
  mutate(p = format_pval(p.value))

# Define simple function for extracting value of interest from
# diversity model results
get_mod_result <- function(resp_var_select, habit_select, 
                           indep_var_select, value) {
  model_summ_print %>% 
    filter(
      resp_var == resp_var_select, 
      habit == habit_select, 
      indep_var == indep_var_select) %>% 
    pull(!!enquo(value))
}
...
<!-- NEED TO UPDATE TO REFLECT mean_RH instead of min_RH -->
On a single-trait basis, distribution of community-weighted mean values (CWMs) showed significant correlation with elevation for some traits, but no significant relationship was detected for any traits with minimum RH (\fig \@ref(fig:cwm-div)). 
Specific leaf area decreased with elevation for epiphytic (linear model, \rval^2^ = `r get_mod_result("sla", "epiphytic", "el", "r")`, \pval = `r get_mod_result("sla", "epiphytic", "el", "p")`) and terrestrial (linear model, \rval^2^ = `r get_mod_result("sla", "terrestrial", "el", "r")`, \pval = `r get_mod_result("sla", "terrestrial", "el", "p")`) communities. 
Degree of lamina dissection decreased with elevation in epiphytes (linear model, \rval^2^ = `r get_mod_result("dissection", "epiphytic", "el", "r")`, \pval = `r get_mod_result("dissection", "epiphytic", "el", "p")`), but not terrestrial communities. 
Stipe length showed a small, but significant, increase with elevation for epiphytic communities (linear model, \rval^2^ = `r get_mod_result("stipe", "epiphytic", "el", "r")`, \pval = `r get_mod_result("stipe", "epiphytic", "el", "p")`). 
Rhizome diameter decreased with elevation for terrestrial communities (linear model, \rval^2^ = `r get_mod_result("rhizome", "terrestrial", "el", "r")`, \pval = `r get_mod_result("rhizome", "terrestrial", "el", "p")`), but increased slightly in epiphytes (linear model, \rval^2^ = `r get_mod_result("rhizome", "epiphytic", "el", "r")`, \pval = `r get_mod_result("rhizome", "epiphytic", "el", "p")`).

We observed a total of `r traits_with_abun %>% filter(abundance > 0) %>% pull(species) %>% n_distinct` species across all plots, including `r n_epi` epiphytic and `r n_ter` terrestrial species. Mean species richness was `r mean_rich %>% filter(habit == "epiphytic") %>% pull(stat)` \si{\species}per plot for epiphytes and `r mean_rich %>% filter(habit == "terrestrial") %>% pull(stat)` \si{\species} per plot for terrestrial species (not significantly different; two-sided \tval-test, \pval = `r t_test_results_print %>% filter(resp_var == "ntaxa") %>% pull(p)`; \fig \@ref(fig:comm-div)). Species richness increased linearly with elevation and minimum RH for epiphytes, but not terrestrial communities (\fig \@ref(fig:comm-div)). Trait richness (FRic) was lower in epiphytic vs. terrestrial communities (two-sided \tval-test, \pval = `r t_test_results_print %>% filter(resp_var == "FRic") %>% pull(p)`), whereas functional diversity (FDiv) was lower in terrestrial communities (two-sided \tval-test, \pval = `r t_test_results_print %>% filter(resp_var == "FDiv") %>% pull(p)`), and functional evenness (FEve) did not differ between the two. FEve decreased with elevation, but not minimum RH, in terrestrial communities (\fig \@ref(fig:comm-div)). Epiphytic communities showed a trend of increasing trait richness along the environmental gradient, with greater FRic at high elevations (linear model, \rval^2^ = `r get_mod_result("FRic", "epiphytic", "el", "r")`, \pval = `r get_mod_result("FRic", "epiphytic", "el", "p")`) and plots with greater minimum RH (linear model, \rval^2^ = `r get_mod_result("FRic", "epiphytic", "mean_RH", "r")`, \pval = `r get_mod_result("FRic", "epiphytic", "mean_RH", "p")`). Epiphytic communities were more phylogenetically clustered than terrestrial communities overall when measured using MNTD (two-sided \tval-test, \pval = `r t_test_results_print %>% filter(resp_var == "mntd.obs.z") %>% pull(p)`) but not MPD (\pval = `r t_test_results_print %>% filter(resp_var == "mpd.obs.z") %>% pull(p)`; \fig \@ref(fig:comm-div)). No significant trend was detected in phylogenetic community structure with environment in either terrestrial or epiphytic communities (\fig \@ref(fig:comm-div)). 
