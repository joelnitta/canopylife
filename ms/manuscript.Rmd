...
Prior to testing for differences in diversity metrics, we first checked for correlation between metrics with Pearson's correlation coefficient, and retained a set of metrics with absolute correlation coefficients < 0.9. We tested for differences between epiphytic and terrestrial communities with a paired \tval-test. We simultaneously tested the effects of environment and growth habit on diversity metrics using a full-subsets multiple regression approach with general additive models (GAMs) as implemented in the \package{FSSgamm} R package [@Fisher2018]. For each diversity metric, we built a set of models (Gaussian family, with site as a random effect) taking into account all combinations of explanatory variables and their pairwise interactions, and ranked these by corrected AIC (AICc) to find the models that best explain the data. Our dataset included `r nrow(div_metrics_select) - nrow(climate_select)` communities that lacked climate data and could not be included in full-subsets multiple regression analysis. Since we found that all climatic variables are related to elevation (see Results), we also constructed linear models for each diversity metric using same model selection procedure as in the climate analysis.
