...
We calculated mean daily mean, minimum, and standard variation of temperature and RH for each site from the raw climate data, then grand means from the daily means. We analyzed correlation between the grand means with Pearson's correlation coefficient, and retained a set with correlation coefficients less than 0.9 (hereafter, "climatic variables"). We constructed models to test for differences in response of climatic variables to elevation between epiphytic and terrestrial dataloggers. For each retained climatic variable, we constructed three linear models: one with growth habit (epiphytic vs. terrestrial) only, one with elevation only, and one with the interaction of elevation and growth habit. We compared the fit of the models using the Akaike Information Criteron (AIC) [@Akaike1973], and kept the model with the smallest AIC for each climatic variable. A single outlier site located on an exposed south-facing slope with unusually high temperature and low humidity (Mt. Rotui, 830 m) site was excluded from analysis.
