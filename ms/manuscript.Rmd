...
In the full-subsets analysis including all climate variables and diversity metrics, growth habit and temperature emerged as the most important variables (\fig \ref{fig:heatmap}). Most of the the best-fitting models included growth habit or the interaction of growth habit and a temperature-related variable (\tab S5). Community-weighted means of frond length, frond width, stipe length, and rhizome diameter were correlated (Pearson's correlation coefficient > 0.9), so of these we only analyzed stipe length.

Distribution of community-weighted mean values (CWMs) differed between epiphytic and terrestrial communities overall (pinna number, stipe length) or co-varied with elevation (dissection, SLA) (\fig \@ref(fig:cwm-div)). Specific leaf area decreased with elevation, and was lower for epiphytic relative to terrestrial communities (linear model, \rval^2^ = `r get_mod_result(model_summ_print, "sla", "r")`, \pval = `r get_mod_result(model_summ_print, "sla", "p")`). Degree of lamina dissection decreased with elevation in epiphytes, but slightly increased in terrestrial communities (linear model, \rval^2^ = `r get_mod_result(model_summ_print, "dissection", "r")`, \pval = `r get_mod_result(model_summ_print, "dissection", "p")`). Stipe length and pinna number showed no significant variation with elevation, but were much lower overall for epiphytes relative to terrestrial communities (\tval-test, both \pval = `r get_mod_result(t_test_print, "stipe", "p")`). 

We observed a total of `r traits_with_abun %>% filter(abundance > 0) %>% pull(species) %>% n_distinct` species across all plots, including `r n_epi` epiphytic and `r n_ter` terrestrial species. Mean species richness was `r mean_rich %>% filter(habit == "epiphytic") %>% pull(stat)` \si{\species}per plot for epiphytes and `r mean_rich %>% filter(habit == "terrestrial") %>% pull(stat)` \si{\species} per plot for terrestrial species (not significantly different; two-sided \tval-test, \pval = `r get_mod_result(t_test_print, "ntaxa", "p")`; \fig \@ref(fig:comm-div)). Species richness increased  with elevation for both epiphytes and terrestrial communities (\fig \@ref(fig:comm-div)). Trait richness (FRic) was lower in epiphytic vs. terrestrial communities (two-sided \tval-test, \pval = `r get_mod_result(t_test_print, "FRic", "p")`), whereas functional diversity (FDiv) was lower in terrestrial communities (two-sided \tval-test, \pval = `r get_mod_result(t_test_print, "FDiv", "p")`), and functional evenness (FEve) did not differ between the two. Both epiphytic and terrestrial communities showed a trend of increasing trait richness along the environmental gradient, with greater FRic at high elevations (linear model, \rval^2^ = `r get_mod_result(model_summ_print, "FRic", "r")`, \pval = `r get_mod_result(model_summ_print, "FRic", "p")`). Epiphytic communities were more phylogenetically clustered than terrestrial communities overall when measured using MNTD (two-sided \tval-test, \pval = `r get_mod_result(t_test_print, "mntd.obs.z", "p")`) but not MPD (\pval = `r get_mod_result(t_test_print, "mpd.obs.z", "p")`; \fig \@ref(fig:comm-div)). No significant trend was detected in phylogenetic community structure with elevation in either terrestrial or epiphytic communities (\fig \@ref(fig:comm-div)). 
...
Results for functional trait richness (FRic) supported our hypotheses: we observed much lower FRic in epiphytic relative to terrestrial communities overall, and a significant positive correlation of FRic with elevation (\fig \@ref(fig:comm-div)). However, results for the other two measures of functional diversity, evenness (FEve) and divergence (FDiv) did not match our predictions. Rather, FDiv was lower in terrestrial communities overall but did not show any trend along the gradient, while FEve did not show any significant differences at all (\fig \@ref(fig:comm-div)). Functional divergence is similar to richness, but takes into account abundances; so the FDiv results may be driven by a small number of abundant species. The lack of a clear pattern in FEve suggests that within each trait pool, there is no significant difference in trait spacing between epiphytes and terrestrial species. This could be maintained by niche partitioning within epiphytic and terrestrial habitats. Epiphytic habitats are highly heterogeneous, and microclimatic conditions vary with height along the host and distance from the trunk [@Hietz1995]. The vertical stratification of epiphytic species into different zones along height gradients in host trees is well documented [@Zotz2007]. In a study comparing terrestrial and epiphytic ferns along an elevational gradient from \SIrange{30}{3000}{\meter} in Costa Rica, @WatkinsJr.2006a found that epiphytes tended to sort into low-trunk and canopy zones, with only 18\% found in both, and no species that overlapped between canopy and terrestrial habitats. It is possible that epiphytic growth per se limits overall morphological variation in epiphytic ferns, but that species with different traits are then sorted into different niches.

In addition to their decreased functional diversity, we also found that epiphytic communities are more phylogenetically clustered relative to terrestrial communities when measured with MNTD, but not MPD (\fig \@ref(fig:comm-div)). This makes sense given that most epiphytes have radiated within five major clades [@Schuettpelz2009]; each epiphytic community we sampled tends to include closely-related species from two or more epiphytic clades. However, we did not detect significant trends in phylogenetic community structure of epiphytic and terrestrial communities along the elevational gradient. It is possible that the scale of the current study (from \SIrange{200}{1200}{\meter}) prevented the detection of such trends. When the plots in this study were analyzed together with additional plots from Tahiti reaching a maximum altitude of just over \SI{2000}{\meter}, @Nitta2017 detected increasing phylogenetic clustering in fern sporophyte communities (epiphytes and terrestrial species together) with elevation. @Kluge2011 also compared phylogenetic and trait diversity between epiphytic and terrestrial ferns over an elevational gradient from \SIrange{100}{3400}{\meter} in Costa Rica, and found trait overdispersion at mid-elevations for epiphytes but no trends in phylogenetic diversity for either group. They accounted this to evolutionary plasticity in traits, but did not quantify the degree of phylogenetic signal in the traits they studied.
...
```{r cwm-div, fig.height = 8, fig.width = 4, fig.cap = "Community-weighted means."}
combined_cwm_plots
```
