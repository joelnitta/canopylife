...
\noindent\fig \ref{fig:CWM}. Mean values of selected functional morphological traits of epiphytic and
...
significant relationships determined by a linear model at $\pval<0.05$. B) Mean values for all
...
statistical significance of one-sided \tval-test; $*** = \pval<0.001$; $** = \pval<0.01$; $* = \pval<0.05$.
...
each). Trendlines indicate significant relationships determined by a linear model at $\pval<0.05$. B) Median values (box plots) of response variables by growth habit (epiphytic or
terrestrial) across all communities (epiphytic communities, $\n = 17$; terrestrial
communities, $\n = 17$). Medians shown with bold lines, lower and upper hinges
